 ...
 */
static void splitout_queryargs(request_rec *r, int qsappend, int qsb64)
{
    char *q;
    ...
    if (q != NULL) {
        ...
        *q++ = '\0';
        ...
        olduri = apr_pstrdup(r->pool, r->filename);
        oldargs = apr_pstrdup(r->pool, r->args);

        apr_sha1_init(&ctx);
        apr_sha1_update(&ctx, r->filename, strlen(r->filename));
        if (r->args && strlen(r->args)) {
            apr_sha1_update(&ctx, "?", 1);
            apr_sha1_update(&ctx, r->args, strlen(r->args));
        }
        apr_sha1_final(digest, &ctx);

        for (i = 0; i < APR_SHA1_DIGESTSIZE; i++) {
            b64name[2 * i] = h2a((digest[i] >> 4) & 0xf);
            b64name[2 * i + 1] = h2a(digest[i] & 0xf);
        }
        b64name[40] = '\0';

        r->filename = apr_pstrcat(r->pool, "/", b64name, NULL);
        r->args = NULL;

        rewritelog((r, 3, NULL, "modified uri=%s, args=%s -> uri=%s", olduri,
                    oldargs, r->filename));
    }
                                           ...
                                           char *key, char *val)
{
    ...
    case 'Q':
        ...
        else if (! strcasecmp(key, "sb64")) {
            cfg->flags |= RULEFLAG_QSB64;
        }
        else {
            ++error;
        }
...
static int apply_rewrite_rule(rewriterule_entry *p, rewrite_ctx *ctx)
{
    ...
    splitout_queryargs(r, p->flags & RULEFLAG_QSAPPEND, p->flags & RULEFLAG_QSB64);
